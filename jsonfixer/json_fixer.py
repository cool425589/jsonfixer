...
class JsonRepair:
    ...
    def repair(self) -> str:
        ...
        print("error self.unexpected_character()")
        ...
        if not self.is_end_of_document() and self.char() == "{":
            ...
            while not self.is_end_of_document() and self.char() != "}":
                ...
                if not processed_key:
                    ...
                    print("error self.object_key_expected()")
                ...
                if not processed_colon:
                    if is_start_of_value(self.char()):
                        ...
                        print("error self.colon_expected() ")
                processed_value = self.parse_value()
                if not processed_value:
                    if processed_colon:
                        # raise self.object_value_expected()
                        print("error  self.object_value_expected()")
                    # raise self.colon_expected()
                    print("error self.colon_expected()")
    ...
    def parse_newline_delimited_json(self):
        """Parse and repair Newline Delimited JSON :
                    ...
                    if self.char() == "\\":
                        ...
                        if escape_char:
                            ...
                            if (
                                ...
                                print("error self.invalid_unicode_character(self.i)")
                        else:
                        ...
                        if char == '"' and self.char(pos=-1) != "\\":
                            ...
                            if not is_valid_string_character(char):
                                ...
                                print("error self.invalid_character(char) ")
                            self.output += char
                            self.inc()
